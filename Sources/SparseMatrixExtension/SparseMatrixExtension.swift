...
extension SparseMatrix_Float{
    public struct Triplet{
        ...
        public init(row: Int, col: Int, value: Float) {
            self.row = row
            self.col = col
            self.value = value
        }
    }
    public static func fromTriplets(triplets:[Triplet],rowCount:Int32,columnCount:Int32)->SparseMatrix_Float{
        let attributes = SparseAttributes_t()
    ...
    public static func concat(a:SparseMatrix_Float,b:SparseMatrix_Float,axis:ConcatAxis)->SparseMatrix_Float{
        switch axis{
        case .row: do {
            return SparseMatrix_Float.fromTriplets(triplets: a.toTriplets() + b.toTriplets(), rowCount: a.structure.rowCount + b.structure.rowCount, columnCount: a.structure.columnCount)
        }
        case .column: do {
            return SparseMatrix_Float.fromTriplets(triplets: a.toTriplets() + b.toTriplets(), rowCount: a.structure.rowCount + b.structure.rowCount, columnCount: a.structure.columnCount)
        }
